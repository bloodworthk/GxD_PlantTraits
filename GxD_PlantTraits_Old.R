...
#all correlations
chart.Correlation(CWM_Collected_Data_FK[9:12],pch="41", cex = 4, method="spearman", histogram = TRUE)

#FK - Height CWM
CWM_Height_Norm_19 <- lm(data = subset(CWM_Collected_Data_FK, year == 2019 & Site== "FK"), (Height_CWM)  ~ rainfall_reduction)
ols_plot_resid_hist(CWM_Height_Norm_19) 
ols_test_normality(CWM_Height_Norm_19) #right skewed

#make chart correlation with just traits we discussed
#transformed
chart.Correlation(CWM_Collected_Data_FK[14:17],pch="41", cex = 4, method="spearman", histogram = TRUE)
#not transformed
...
Height_FK_ALL<-ggplot(subset(CWM_Collected_Data_avg,Site=="FK"&year>=2019),aes(x=rainfall_reduction,y=Height_CWM_Mean,color=as.factor(year),shape=as.factor(year))) +  
  ...
  scale_color_manual(values=c("darkslateblue","blue4","maroon4","darkgreen","yellow"),labels = c("2019", "2020","2021","2022","2023"), breaks = c("2019","2020","2021","2022","2023"),name="Year")+
...
#CWM of height for Fort Keogh 2023 - LMER
FK_Height_2023_LMER_slope <- lmerTest::lmer(data = subset(CWM_Collected_Data,year==2023&Site=="FK"), Height_CWM ~ grazing_treatment*Rainfall_reduction_cat + (1|block) + (1|block:slope))
anova(FK_Height_2023_LMER_slope, type = 3) #drought (p=0.02882)
...
#CWM of LeafThickness for Fort Keogh 2023 LMER
FK_LeafThickness_2023_LMER_slope_slope <- lmerTest::lmer(data = subset(CWM_Collected_Data,year==2023&Site=="FK"), LeafThickness_CWM ~ grazing_treatment*Rainfall_reduction_cat + (1|block) + (1|block:slope))
anova(FK_LeafThickness_2023_LMER_slope_slope, type = 3) #ns
...
#CWM of LDMC for Fort Keogh 2023 - LMER
FK_LDMC_2023_LMER_slope_slope <- lmerTest::lmer(data = subset(CWM_Collected_Data,year==2023&Site=="FK"), LDMC_CWM ~ grazing_treatment*Rainfall_reduction_cat + (1|block) + (1|block:slope))
anova(FK_LDMC_2023_LMER_slope_slope, type = 3) #ns
...
#CWM of Area for Fort Keogh 2023 - LMER
FK_Area_2023_LMER_slope <- lmerTest::lmer(data = subset(CWM_Collected_Data,year==2023&Site=="FK"), Area_CWM ~ grazing_treatment*Rainfall_reduction_cat + (1|block) + (1|block:slope))
